...
NOTE: This is a modified version of the original document from Michalis, because we (authors of http://newpascal.org/ and http://synopse.info/ ) prefer the "mode delphi" without the "generic" / "specialize" keywords.
...
The _logical (or bit-wise) operators have a higher precedence than relational operators_. So you may need to use parenthesis around some expressions.
...
* Analogous rule is for `or` expression. There, if the expression is known to be `true` (because the 1st operand if `true`), the 2nd operand is not evaluated.
...
This is a "magic" routine in Pascal. It takes a variable number of arguments and they can have any type. They are all converted to strings when displaying, with some special syntax for specifying padding and number precision.
...
To explicitly use newline in string, use the `LineEnding` constant (FPC RTL) (the _Castle Game Engine_ has also a shorter `NL` constant). Pascal strings do not interpret any special backslash sequences, so writing
...
*In the Castle Game Engine:* use `WritelnLog` or `WritelnWarning`, never `Writeln`. They will be always directed to some useful output. On Unix, standard output. On Windows GUI application, log file. On Android, the _Android logging facility_ (visible when you use `adb logcat`).

### Converting to a string

If you're looking for a way to convert an arbitrary number of arguments to a string (instead of just directly outputting them), you have a couple of options.
...
** _Another advantage_: Consistent with the reverse functions. To convert a string (for example, user input) back to an integer or float, you use `StrToInt`, `StrToFloat` or friends (like `StrToIntDef`).
...
** _Disadvantage_: Compiler does not check whether the pattern matches the arguments. Using a wrong placeholder type will result in an exception at runtime (`EConversionError` exception, not anything nasty like a segmentation fault).
...
The unit `Graphics` (from Lazarus LCL) defines the `TColor` type. But the compiler will fail to compile the above unit, claiming that you don't implement a procedure `ShowColor` that matches the interface declaration. The problem is that unit `GoogleMapsEngine` also defines a `TColor` type. And it is used only in the `implementation` section, therefore it _shadows_ the `TColor` definition only in the implementation. The equivalent version of above unit, where the error is obvious, looks like this:
...
// INCORRECT example.
// This is what compiler "sees" when trying to compile previous example
...
Sometimes you need to take an identifier from a used unit, and _expose_ it from your new unit. The end result should be that using the new unit should make the identifier available in the namespace.
...
begin
  ...
  { With O2, things are more difficult. It's not nil, but it's invalid.
    Trying to access a field of a non-nil invalid instance (or call a method
    that accessed a field of such instance) results in an unpredictable
...
For records, it's adviced to use `{$modeswitch advancedrecords}` and override operators as `class operator` inside the record (for delphi mode available by design). This allows to use generic classes that depend on some operator existence (like `TFPGList`, that depends on equality operator being available) with such records. Otherwise the "global" definition of an operator (not inside the record) would not be found (because it's not available at the code that implements the `TFPGList`), and you could not specialize a list like `TFPGList<TMyRecord>`.

NOTE: for TList from Generics.Collections module you don't need to override operators.
...
The source code of this document without the "generic" / "specialize" keywords on https://github.com/maciej-izak/modern-pascal-introduction .
